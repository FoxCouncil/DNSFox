...
namespace PresentationControls
{
    ...
    public class ObjectSelectionWrapper<T> : INotifyPropertyChanged
    {
        ...
        /// <summary>
        /// Cleans up Property String fetching
        /// </summary>
        /// <returns></returns>
        private string GetPropertyInfo()
        {
            PropertyInfo PI = Item.GetType().GetProperty(_Container.DisplayNameProperty);
            if (PI == null) throw new Exception(String.Format("Property {0} cannot be found on {1}.", _Container.DisplayNameProperty, Item.GetType()));
